...
<section class="content">
    <div class="container-fluid">
        ...
        @if(count($product_categories)<1) <div class="row">
            <div class="col-sm-6 text-left mb-2">
                ...
                @if(isset($store_products_count) && $store_products_count <= 0) <button type="button" class="btn btn-success" disabled>Sync Categories</button>
        ...
        <table class="table" id="store_product_table">
            <thead>
                <tr>
                    ...
                    <th width="40%" scope="col">Categories</th>
            ...
            <tbody>
                ...
                @if(count($product_categories)<1) <tr class="trbody pl-5">
                    <td>
                    ...
                    <tr class="trbody" data-row_product_category="{{$category->id}}">
                        ...
                        <td>
                            @if(count($storefront_categories)<1) <h6>No categories to map</h6>
                                @else
                                <select class="js-select2-multi select_category_mapping" id="" multiple="multiple">
                                    ...
                                    <option value={{$storefront_category->id}} data-product_category="{{$category->id}}" data-store_id="{{$category->store_id}}" selected>
...
<script>
    ...
    $(document).on('click', '.category_mapping', function() {
        ...
        $.ajax({
            ...
            type: 'post',
            data: {
                storefront_categories: selected_storefront_categories,
            ...
            success: function(response) {
                ...
                if (response == "mapping_saved") {
                    toastr.success('Mapping has been saved successfully.', {
                        timeOut: 5000
                    });
                }
                // validation current user has no store
                else if (response == "mapping_removed") {
                    toastr.success('Mapping has been removed successfully.', {
                        timeOut: 5000
                    });
                }
            },
